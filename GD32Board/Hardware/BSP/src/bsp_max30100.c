...
//写数据到MAX30100寄存器
void MAX30100_WriteReg(uint8_t reg_add,uint8_t reg_dat)
{
	i2c_Start();
...
//从MAX30100寄存器读取数据
void MAX30100_ReadData(uint8_t reg_add,uint8_t *Read,uint8_t num)
{
	unsigned char i;
...
void MAX30100_FIFO_ReadData(uint8_t *Read)
{
	i2c_Start();
	i2c_SendByte(MAX30100_SLAVE_ADDRESS);
	i2c_WaitAck();
...
void ReadID(void)
{
	uint8_t ID,Revision;//分别对应硬件ID和修订ID
...
{
	float temp;
	uint8_t tempInteger,tempFraction;  						 //分别对应温度整数部分和小数部分
	MAX30100_WriteReg(ModeConfiguration,0x0b); //复位芯片，设置模式 
...
//	MAX30100_ReadData(InterruptStatus,&status,1);
 //这里可以加中断标志位校验if(...)
	...
	ir_value = ir_value / 500;
	if(num++ > 15)
	{	
		if(num == 16)
			ir_value_tmp = ir_value;
		else
		{
			num = 17;
			if(abs(ir_value - ir_value_tmp) > 30)
			{
				ir_value = ir_value_tmp;
			}
			else
				ir_value_tmp = ir_value;
		}
